...
const TableOfContents = memo(({ contentRef }: { contentRef: React.RefObject<HTMLDivElement | null> }) => {
    ...
    useEffect(() => {
        ...
        const findAndExpandTrail = (items: any[], trail: string[] = []): boolean => {
            for (const item of items) {
                const newTrail = [...trail, item.id];

                if (item.id === activeId) {
                    newTrail.forEach(id => newExpanded[id] = true);
                    return true;
                }
        ...
        setExpandedItems(newExpanded); // Only keep active trail expanded
    }, [activeId, toc]);
