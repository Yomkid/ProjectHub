...
import { Bookmark, Share2, ThumbsUp, Eye, Printer, Quote } from 'lucide-react';
import { PDFDownloadLink } from '@react-pdf/renderer';
import Head from 'next/head';
import CitationBox from '@/components/CitationBox';
import Modal from '@/components/CitationModal';

// In your ArticlePage component
import { ArticlePDF } from '@/components/ArticlePDF';

// Replace your current downloadArticleAsPDF function with this:
const extractArticleContent = (contentRef) => {
        ...
        return {
            ...
            sections: Array.from(articleElement.querySelectorAll('section')).map(section => {
                ...
                contentBlocks.forEach(block => {
                    ...
                    else if (block.classList.contains('bg-gray-100')) { // Code block
                        sectionData.subsections.push({
                            code: block.querySelector('pre')?.textContent || ''
                        });
                    }
                    else if (block.classList.contains('text-center')) { // Image block
                        sectionData.subsections.push({
                            image: block.querySelector('img')?.src || '',
                            imageCaption: block.querySelector('p')?.textContent || ''
                        });
                    }
                    else if (block.id && block.id !== '') { // Regular subsection
                        const subsection = {
    ...
    const TableOfContents = memo(({ contentRef }: { contentRef: React.RefObject<HTMLDivElement | null> }) => {
        ...
        }, [activeId, expandedItems, toggleExpand]);
    ...
    export default function ArticlePage() {
        ...
        //     const opt = {
        //         margin: 0.5,
        //         filename: 'face-recognition-article.pdf',
                    ...
                    <div className="lg:w-2/4 bg-white shadow-md p-6 mb-6 flex flex-col" ref={contentRef}>
                        ...
                        <div className="flex flex-col gap-2 mb-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                <PDFDownloadLink
                                    document={<ArticlePDF content={extractArticleContent(contentRef)} />}
                                    fileName="face-recognition-article.pdf"
                                >
                                    {({ loading }) => (
                                        <button
                                            className={`bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 self-start mb-4 flex items-center gap-2 ${loading ? 'opacity-75 cursor-not-allowed' : ''
                                                }`}
                                            disabled={loading}
                                        >
                                            {loading ? (
