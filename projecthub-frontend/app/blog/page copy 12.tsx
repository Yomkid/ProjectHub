...
const TableOfContents = memo(({ contentRef }: { contentRef: React.RefObject<HTMLDivElement | null> }) => {
    ...
    useEffect(() => {
        ...
        const expandedTrail: Record<string, boolean> = {};
    
        // Recursive function to find the active item and mark parents
        const findAndExpandTrail = (items: any[], trail: string[] = []) => {
            for (const item of items) {
                const newTrail = [...trail, item.id];
                if (item.id === activeId) {
                    newTrail.forEach(id => expandedTrail[id] = true);
                    return true;
                }
                if (item.children && findAndExpandTrail(item.children, newTrail)) {
                    return true;
                }
            }
            return false;
        ...
        findAndExpandTrail(toc);
        setExpandedItems(prev => ({ ...prev, ...expandedTrail }));
    }, [activeId, toc]);
