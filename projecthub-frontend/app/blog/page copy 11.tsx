...
import { Bookmark, Share2, ThumbsUp, Eye, Printer, Quote, CheckCircle } from 'lucide-react';
...
const TableOfContents = memo(({ contentRef }: { contentRef: React.RefObject<HTMLDivElement | null> }) => {
    ...
    useEffect(() => {
        ...
        Array.from(headings).forEach((heading) => {
            const level = parseInt(heading.tagName[1]); // Get numeric level (1-6)
            ...
            // Find where this item should be nested
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            ...
            {
                rootMargin: '-90px 0px -70% 0px', // Top margin to account for navbar height
    ...
    const NAVBAR_OFFSET = -90; // Adjust based on your fixed navbar height
                    ...
                    <div className="flex items-center justify-between group">
                        <a
                            ...
                            onClick={(e) => {
                                ...
                                if (target) {
                                    ...
                                    setActiveId(item.id); // Optional: set manually on click
                                }
                            }}
                            className={clsx(
                        ...
                        {item.children?.length > 0 && (
                            <button
                                ...
                                className="ml-1 text-sm text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-1 focus:ring-green-300 rounded transition"
                            >
                                {isExpanded ? '−' : '+'}
                            </button>
                        )}
