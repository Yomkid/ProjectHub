...
const TableOfContents = memo(({ contentRef }: { contentRef: React.RefObject<HTMLDivElement | null> }) => {
    ...
    useEffect(() => {
        ...
        Array.from(headings).forEach((heading) => {
            ...
            const slug = heading.textContent?.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '') ?? '';
            heading.id = slug;
    ...
    const renderTocItems = useCallback((items: any[], level = 0) => {
        return items.filter(item => item.text?.toLowerCase().includes(searchQuery.toLowerCase())).map((item, index) => {
                    ...
                    <div className="flex items-center justify-between group">
                        <a
                            ...
                            className={clsx(
                                ...
                                isActive ? 'bg-green-50 text-green-600 font-medium' : 'text-gray-700 hover:text-green-600',
                                paddingLevel
                            )}
                        >
                            {item.text}
    ...
    return (
        <>
            ...
            <button
                ...
                className="fixed bottom-6 right-6 z-50 md:hidden bg-green-600 text-white rounded-full p-3 shadow-lg"
            >
            ...
            <div className={clsx(
                'fixed inset-0 bg-white z-50 overflow-y-auto transition-transform duration-300 md:hidden',
                isOpen ? 'translate-x-0' : 'translate-x-full')
            }>
                <div className="p-4">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="flex items-center text-xl font-semibold mb-4 text-gray-800">
                            <ListTree className="w-5 h-5 mr-2 text-gray-600" />
                            Table of Contents
                        </h3>
                    ...
                    />
                    {toc.length > 0 ? renderTocItems(toc) : <p className="text-gray-500 text-sm">No headings found</p>}
                </div>
            </div>
...
export default function ArticlePage() {
    ...
    return (
        <>
            <div className="max-w-7xl mx-auto p-4 flex flex-col gap-6 lg:flex-row">
                <Head>
                    ...
                    <script type="application/ld+json">
                        {JSON.stringify({
                            ...
                            "author": { "@type": "Person", "name": "Ismail Mainbay" },
                ...
                {/* Left Sidebar (TOC) - hidden on mobile, visible on lg+ */}
                <TableOfContents contentRef={contentRef} />

                {/* Main Content */}
                <div className="w-full lg:w-2/4 bg-white shadow-md p-4 sm:p-6 mb-6 flex flex-col" ref={contentRef}>
                    <img
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center text-sm text-gray-500 mb-4 gap-2">
                        <span className="flex items-center gap-1">
                            Written by{' '}
                            <a href="#" className="flex items-center font-bold text-green-600 underline">
                                Mayomi Odewaye <CheckCircle className="w-4 h-4 text-green-500 ml-1" />
                            </a>{' '}
                            on Jun 30, 2021
                        </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {[{
                            ...
                            icon: <ThumbsUp size={18} className="text-yellow-500" />,
                            text: "45"
                        }, {
                            icon: <Eye size={18} />,
                            text: "200"
                            ...
                            text: "Cite",
                            action: () => setShowCitation(prev => !prev)
                        }].map((btn, i) => (
                            <button
                                key={i}
                                className="bg-gray-50 px-3 py-1 flex items-center space-x-1 rounded-md shadow-sm"
                                onClick={btn.action}
                            >
                                {btn.icon}
                                {btn.text && <span className="font-semibold">{btn.text}</span>}
                            </button>
                        ))}
                    </div>
